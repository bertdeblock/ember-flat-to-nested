...
async function flatToNested(rootDir, { revert = false } = {}) {
  try {
    ...
    if (componentDirAddonFull) {
      ...
      throw new Error(`"${COMPONENT_DIR.APP}" or "${COMPONENT_DIR.ADDON}" directory not found.`);
    }
    ...
    if (revert) {
      ...
      success(`"${componentDir}" was successfully transformed to a flat structure.`);
    } else {
      await Promise.all(componentFiles.map(flatToNestedComponent));
      success(`"${componentDir}" was successfully transformed to a nested structure.`);
    }
  } catch (exception) {
